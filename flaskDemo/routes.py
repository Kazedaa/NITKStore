...
# Configure db
database = yaml.load(open(r'.\flaskDemo\database.yaml'),Loader=yaml.SafeLoader)

@app.route("/")
@app.route("/home", methods=['GET', 'POST'])
def home():
    products = Product.query.all() #product table
...
def update_product(product_id):
    ...
    if form.validate_on_submit():
        if form.picture.data:
            ...
            picture_file=product.image[len('/static/profile_pics/'):]
            image_file = url_for('static', filename='profile_pics/' + picture_file)
            product.image=image_file
        ...
        form.categoryName.data='null'
        form.productName.data = product.productName
        form.picture.data=product.image
        picture_file=form.picture.data[len('/static/profile_pics/'):]
        image_file=url_for('static', filename='profile_pics/' + picture_file)
